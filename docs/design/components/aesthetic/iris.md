...
- **`config/iris.properties`**（281 B）：当前配置为 `enableShaders=true`、`shaderPack=`（空）、`maxShadowRenderDistance=32`、`allowUnknownShaders=false`、`colorSpace=SRGB`。着色器功能已启用，但未预置任何着色器包，由玩家自行选择安装，符合"可扩展性"支柱——玩家可根据个人硬件条件和审美偏好自由决定是否启用着色器。`maxShadowRenderDistance=32` 限制光影包的最大阴影渲染距离，在视觉效果与性能之间取得平衡。
